...
train_data_dir = './mark_2/train'
test_data_dir = './mark_2/test'
...
nb_train_2_samples = get_filecount("mark_2/train/rate2")
...
nb_train_samples = nb_train_1_samples + nb_train_2_samples


nb_test_1 = get_filecount("mark_2/test/rate1")
nb_test_2 = get_filecount("mark_2/test/rate2")

nb_test_1 = nb_test_1 - nb_test_1 % batch_size
nb_test_2 = nb_test_2 - nb_test_2 % batch_size
nb_test_samples = nb_test_1 + nb_test_2
...
def save_bottlebeck_features():
    print('\n\nentering bottleneck_features...')
    datagen = ImageDataGenerator(rescale=1. / 255,
                                 horizontal_flip=True)

    # build the VGG16 network
    model = applications.inception_resnet_v2.InceptionResNetV2(include_top=False, weights='imagenet', input_tensor=None, input_shape=None, pooling=None, classes=2)
    ...
    np.save(open('bottleneck_features_train_2class_resnet_kf.npy', 'wb'),
    ...
    print("saving test bottleneck features")

    np.save(open('bottleneck_features_test_2class_resnet_kf.npy', 'wb'),
...
def train_top_model():
    ...
    train_data = np.load(open('bottleneck_features_train_2class_resnet_kf.npy', 'rb'))
    train_labels = np.array([0] * int(nb_train_1_samples) + [1] * int(nb_train_2_samples))
    tr_labels = np.array([0] * int(nb_train_1_samples) + [1] * int(nb_train_2_samples))
    ...
    test_data = np.load(open('bottleneck_features_test_2class_resnet_kf.npy', 'rb'))
    test_labels = np.array([0] * int(nb_test_1) + [1] * int(nb_test_2))
    ...
    for train, test in kfold.split(train_data, tr_labels):
        ...
        print('3')
        checkpointer = ModelCheckpoint(filepath='model_class2_resnetv2_dense1_4096_2_kf' + str(i) + '.h5', verbose=1,
                                       save_best_only=True)
        ...
        get_confusion_matrix('model_class2_resnetv2_dense1_4096_2_kf' + str(i) + '.h5',test_data,test_labels)
        i += 1
        # model.save_weights(top_model_weights_path)
            #name = 'Timestamp: {:%Y-%m-%d %H:%M:%S}'.format(datetime.datetime.now())
    model.save('model_final_class2.h5')
    get_confusion_matrix('model_final_class2.h5', test_data, test_labels)
